...
Web 任务派发器：注册 GitLab 仓库 → 选分支 → 派发任务 → 每个任务在独立 git worktree 里跑一个**交互式 Claude Code（tmux 会话）** → 网页内 xterm 直接进终端交互 → 完事归档或清理 worktree。并发多个 feature 互不干扰。
...
远程机器在控制端 db 里是**一等的 host 记录**（本机 = host #0，恒在线）。后台每 ~10s 走 ssh 探活每台远程机（复用 ControlPersist socket，很便宜），更新其 online/offline 状态；某机离线时，**必须在该机上跑的写操作**（注册仓库 / 派发 / 清理…）一律被拒，读操作仍放行。
...
## 用法
1. 左侧「新建仓库」：填名称 + git url（https 私有库可填 token）。点「注册并克隆」，状态变 `ready`。
2. 「派发任务」：选仓库 → 选基分支 → 填标题 + 给 claude 的初始指令 →（可选）挑预设 / 勾额外 skill → 「建 worktree + 起会话」。
3. 右侧终端自动连上该会话，直接跟 claude 交互；任务卡片「进终端」可随时重连。会话卡在权限弹窗等你确认时，卡片亮**黄灯**。
4. 完事后任选：**归档**（杀会话、保留 worktree，进归档页）/ **清理**（杀会话 + 删 worktree）/ **删除**（删任务记录，worktree 还在时拒绝）。
5. 不想建仓库时用「本地快速任务」：直接在本机某目录（默认 `~`）开一个裸 tmux shell，自己 `cd`、跑 claude 或任何命令；同一套列表 / 连接 / 归档 / 删除照用。
...
```
server/
  paths.ts    路径常量 + 一次性 data 目录迁移判定（DID_MIGRATE）
  schema.ts   建表 + 给老库逐列补差（SQLite 无 ADD COLUMN IF NOT EXISTS）+ ./data→~/.task-dispatcher 路径迁移；可跑在任意 sqlite handle（含内存测试库）
  db.ts       better-sqlite3 句柄：repos / tasks / hosts / presets 表 + 行类型 + seed 本机(host #0)
  git.ts      mirror clone / fetch / 列分支 / worktree 生命周期
  runner.ts   本机/远程执行抽象（exec/mkdirp/exists/rmrf/putDir/ptySpec）；远程走 ssh（ControlMaster 复用 + tar-over-ssh 传目录）
  tmux.ts     会话 起/查/杀 + cancelCopyMode（attach 时把 pane 拉回活提示符）
  hooks.ts    注入 per-task hooks——黄灯：卡权限弹窗 touch .claude/waiting，恢复/结束则删；dispatcher 经 Runner 回读（本机 fs / 远程 ssh test -e）
  liveness.ts 后台 ssh 探活远程机器（约 10s/轮，复用 ControlPersist socket），更新 online/offline + data_dir
  manifest.ts 写 repos.json（机器自述的仓库清单，只含元数据、绝不含 token；只写不回读）
  migrate.ts  ./data 搬家后 git worktree repair 修 worktree 链接（尽力而为、幂等）
  local.ts    repo-less「本地快速任务」的 cwd 解析
  ...
  index.ts    REST API（repos / tasks / hosts / skills / presets / plugins + 派发注入）+ /pty WebSocket
...
- **不翻译**：git / node-pty 等外部工具透传的原始报错（动态、非我方文案），保持原样。
