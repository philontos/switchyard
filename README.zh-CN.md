...
窄屏下自动切换成一套完整的触屏体验。电脑端「连接设备」引导会生成私有手机地址和二维码；手机登录同一 Tailscale 账号后扫码即可接入。**推荐的打开方式：Safari 访问 → 分享 → 「添加到主屏幕」→ 勾选「作为网页 App 打开」**——以独立 App 模式运行（无浏览器边框、深色启动底、无白闪），从此手机上点图标就进控制台，体验最接近原生 App。
...
打开 `http://127.0.0.1:4500`，点击「连接设备」。Web 引导会依据本机实时状态逐项检查：Tailscale 安装/登录/私有 HTTPS、接电与空闲睡眠、手机二维码，以及可选的第二台电脑发现与 SSH。远程步骤没完成也不会阻塞本机功能。

> `setup.sh` 依次做三件事：① **环境预检**——用非交互 zsh 验证 `claude` / `kimi` / `tmux` / `git` 可达（任务就是用这种只读 `~/.zshenv` 的 shell 启动的，找不到命令任务面板会直接死），缺的把所在目录幂等写进 `~/.zshenv`；② `npm install`（运行时依赖，零构建）；③ 安装全局 `tdsp` 命令（`~/.task-dispatcher/src` 指向这份 clone，启动器链到 `~/.local/bin/tdsp`——敲不到就把 `~/.local/bin` 加进 PATH）。`--check` 只检查、不写不装。
...
Tailscale 是可选的系统网络层，不是 npm 依赖。普通流程是运行 `tdsp serve` → 打开本机控制台 → 点击「连接设备」。引导会检测 [Tailscale 官方客户端](https://tailscale.com/download)、登录态和 Serve 授权，每次只给出当前下一步；能自动完成时会配置 Switchyard 的私有 HTTPS，并再次校验结果。系统客户端安装和用户账号登录仍由用户在系统/浏览器中明确完成。
...
- **手机 → 控制台：** 经 Tailscale Serve 访问 HTTPS/WebSocket；私有路由真正就绪后，引导才显示二维码，并在同一 Tailscale 登录下的 Safari 成功打开后标记为已接入。
...
同一份引导也会检查开发机是否能保持在线。在 macOS 上，「运行期间保持唤醒」使用仅绑定当前 Switchyard 进程、只在接电时生效的 `caffeinate`：屏幕仍可正常熄灭，Switchyard 退出后保活自动释放。MacBook 合盖是另一层硬件/系统条件；引导会报告当前闭盖运行能力，但不会绕过 macOS 的睡眠保护。需要合盖运行时，应保持接电并满足 Apple 支持的闭盖显示条件。
...
1. **两边启动** —— 每台电脑都独立安装并运行 `tdsp serve --tailscale`，登录同一个 Tailscale 账号。
2. **自动连接** —— 在任一控制台点击机器栏「＋」→「发现设备」→「连接」。双方通过 Tailscale HTTPS 完成身份核验、专用 SSH key 交换和双向登记；此阶段不要求系统已经开启 SSH。
3. **检查执行链路** —— Switchyard 会异步尝试一次 SSH 探测。Remote Login 已开启时设备自动转为在线；尚未开启时，连接关系会保留为「SSH 尚未就绪」，不会回退成由另一台机器代管任务。
4. **用起来** —— 这台机器自己的仓库和实时任务出现在页面上。选**它的**仓库派发（仓库组上的 ＋），或在它上面开 shell（Shells 组的 ＋）。

没有使用 Tailscale 自动发现时，仍可点击「手动添加」，填写名字、SSH 目标和可选的隔离 profile。这是高级回退入口，需要 SSH 已经可达；远端没有 Switchyard 时只提供显式安装/bootstrap，不会替远端持有仓库或任务数据。
...
server/                REST API + /pty WebSocket;tdsp CLI;git / tmux / pty / ssh 的 Runner 编排
  ...
  onboarding/          实时网络/手机/电源/舰队就绪状态与安全常在线动作
