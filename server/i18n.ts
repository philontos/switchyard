...
const messages: Record<Lang, Record<string, string>> = {
  en: {
    ...
    "session.orphanRefused": "session {name} is not tracked by this controller (it may belong to another controller sharing this machine); pass force to kill it anyway",
  ...
  zh: {
    ...
    "session.orphanRefused": "会话 {name} 不属于当前 controller（可能属于共用本机的另一个 controller）；如确需结束请传 force",
    "host.offline": "机器离线",
