...
(function () {
  const DICT = {
    zh: {
      ...
      "runtimeRef.manifestHint": "Ref 会写入任务目录和动态 manifest，不会操作或重启当前会话；Agent 正在回复时也可安全添加。之后可直接使用 ref:别名或仓库名引用。",
      "runtimeRef.noRepos": "当前节点没有其他可引用的仓库",
      "runtimeRef.limit": "当前任务已经有 8 个 Ref",
      "runtimeRef.loadingBranches": "正在加载分支…",
      "runtimeRef.submit": "添加 Ref",
      "runtimeRef.adding": "正在 fetch 并创建引用代码快照…",
      "runtimeRef.attached": "已添加 ref:{alias}；当前会话未被打断",
    ...
    en: {
      ...
      "runtimeRef.manifestHint": "The Ref is written into the task directory and dynamic manifest without touching or restarting the live session, so it is safe to add while the agent is responding. Refer to it later by ref:alias or repository name.",
      ...
      "runtimeRef.submit": "Add Ref",
      "runtimeRef.adding": "Fetching and creating the reference code snapshot…",
      "runtimeRef.attached": "Added ref:{alias}; the current session was not interrupted",
